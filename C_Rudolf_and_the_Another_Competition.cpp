...
//================================================================================================================//
vector<pair<int,int>> maxmin;       // When in vector pair first column is to sorted in descending and      
void maxminsort(vector<pair<int,int>> vp1) // second in desecnding simultaneously.
{
    sort(vp1);  reverse(vp1);   vector<pair<int,int>> vp2;

    int num = vp1[0].first;
    for(int i=0; i<vp1.size(); i++){
        if(vp1[i].first == num){vp2.pb(mp(vp1[i].first, vp1[i].second));}
        else
        {sort(vp2);
            for(int j=0; j<vp2.size(); j++){
                maxmin.pb(mp(vp2[j].first, vp2[j].second));}
            vp2.clear();
            num = vp1[i].first;
            vp2.pb(mp(vp1[i].first, vp1[i].second));}
    }
    if(vp2.size()!=0){ sort(vp2);
        for(int j=0; j<vp2.size(); j++){
            maxmin.pb(mp(vp2[j].first, vp2[j].second));}}   //clear the vector at last;                             
}
//================================================================================================================//
...
signed main()
{
    ...
    testcase
    {
        ...
        for(int i=0; i<n; i++)
        {
            ...
            vp1.pb(mp(score, penalty));
        }

        pair<int,int> p1 = {vp1[0].first, vp1[0].second};
        
        maxminsort(vp1);
        vector<pair<int,int>> vp3 = maxmin;
        maxmin.clear();
        ...
        for(int i=0; i<n; i++)
        {
            if(vp3[i].first == p1.first && vp3[i].second == p1.second)
            {
                ans = i+1;
                break;
            }
